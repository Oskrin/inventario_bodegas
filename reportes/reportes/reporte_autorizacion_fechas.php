...
session_start();
    ...
    while($row=pg_fetch_row($consulta)){
        $repetido=0;
        $total=0;
        $sql1=pg_query("select id_factura_venta,num_factura,num_autorizacion,fecha_autorizacion,fecha_caducidad FROM factura_venta where id_cliente='$row[0]' and estado='Activo' and fecha_autorizacion between '$_GET[inicio]' and '$_GET[fin]' and id_usuario='$_SESSION[id]'");
        if(pg_num_rows($sql1)){
            if($repetido==0){
