...
session_start();
    ...
    while($row=pg_fetch_row($consulta)){
        $repetido=0;
        $total=0;
        $sql1=pg_query("select * from proforma where fecha_actual between '$_GET[inicio]' and '$_GET[fin]' and id_cliente='$row[0]' and estado='Activo' and id_usuario='$_SESSION[id]'");
        if(pg_num_rows($sql1)){
            if($repetido==0){
